...
RPlot11 <- function (data,
  ...
  panel13 <- function(data) {
    ...
    DF$SSREF <-
      -data$THDG + atan2((data$GGVEW - u), (data$GGVNS - v)) * 180 / pi
    DF$SSREF[!is.na(DF$SSREF) & (DF$SSREF < -180.)] <-
      DF$SSREF[!is.na(DF$SSREF) & (DF$SSREF < -180.)] + 360.
    plotWAC (
      DF,
      ylab = expression (paste ("SSRD [", degree, "]")),
      legend.position = 'topright',
      ylim = YLMF (3, range (as.matrix (DF[, c('SSRD', 'SSREF')]), finite =
          TRUE))
    )
    title (sprintf (
      "mean diff SSRD-SSREF = %.02f",
